...
if __name__ == '__main__':
    ...
    # 获取历史k线数据
    # all_data  #所有k线数据
    ret, all_data, page_req_key = quote_ctx.request_history_kline(TRADING_CODE, start=START_DATE, end=END_DATE, max_count=1000)
    if ret != RET_OK:
        print('获取K线失败：', all_data)

    while page_req_key != None:  # 请求后面的所有结果
        ret, data, page_req_key = quote_ctx.request_history_kline(TRADING_CODE, start=START_DATE, end=END_DATE, max_count=1000, page_req_key=page_req_key)  # 请求翻页后的数据
        if ret == RET_OK:
